...
def make_mix_ood(dataset, n_rows=None, external_path= "../datasets/YearPredictionMSD.txt", 
    ...
    if external_path == '../datasets/slice_localization_data.csv':
        external_data = pd.read_csv(external_path, header=0)
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        if dataset == "YearPredictionMSD":
            make_mix_ood(dataset, n_rows=51630, external_path='../datasets/slice_localization_data.csv')
        else:
